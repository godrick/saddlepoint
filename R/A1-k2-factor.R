...
.weighted_gram_chol_atomic <- local({
  forward <- function(x) {
    ...
    if (has_dense == 0L) {
      # Pure factors remain in square-root arithmetic.  Forming F F' here is
      # the cancellation-prone operation this terminal exists to avoid.
      ...
      if (!identical(dim(R_pivoted), c(m, m)) || any(!is.finite(R_pivoted))) {
        return(bad)
      }

      singular_values <- tryCatch(
        base::svd(R_pivoted, nu = 0L, nv = 0L)$d,
        error = function(e) NULL
      )
      if (is.null(singular_values) || any(!is.finite(singular_values)) ||
          max(singular_values) <= 0 ||
          min(singular_values) <=
            max(1L, m, r) * .Machine$double.eps * max(singular_values)) {
        return(bad)
      }

      # Undo LAPACK's coordinate pivot before returning the ordinary-coordinate
      # positive-diagonal Cholesky factor.  This second QR canonicalizes an
      # already certified root; it does not make the rank decision.
      ...
      S_scale <- max(abs(S))
      symmetry_tolerance <-
        10 * max(1L, m, r) * .Machine$double.eps * S_scale
      ...
      reference <- 0.5 * (reference + t(reference))
      ...
      if (is.null(singular_values) || any(!is.finite(singular_values)) ||
          max(singular_values) <= 0 ||
          (min(singular_values) / max(singular_values))^2 <=
            max(1L, m, r) * .Machine$double.eps) {
        return(bad)
      }
    }
    ...
    reconstruction_error <-
      max(abs(base::crossprod(R_scaled) - reference)) / max(abs(reference))
    reconstruction_tolerance <-
      100 * max(1L, m, r) * .Machine$double.eps
...
# Values can be obtained from a square root under a root-level rank check, but
# covariance solves and second derivatives of logdet need a covariance-level
# condition check to avoid silently losing every meaningful digit.
#' @keywords internal
.K2_solve_root_guard_atomic <- local({
  forward <- function(x) {
    x <- .rtmb_value_real(x)
    m <- as.integer(sqrt(length(x)))
    bad <- rep(NaN, length(x))
    if (m < 1L || m * m != length(x) || any(!is.finite(x))) return(bad)
    R <- base::matrix(x, nrow = m, ncol = m)
    singular_values <- tryCatch(
    ...
    if (!is.finite(rho_K) ||
        rho_K <= 64 * max(1L, m) * .Machine$double.eps) return(bad)
    x
  }
  ...
  RTMB::ADjoint(forward, reverse, name = "K2_solve_root_guard")
})

#' @keywords internal
.K2_solve_root_guard <- local({
  atomic <- .K2_solve_root_guard_atomic
  function(R) {
    out <- atomic(as.vector(R))
    attr(out, "dim") <- dim(R)
    out
  }
})

# Certify the actual scaled solve.  The atomic is the identity on every valid
# evaluation, so its pullback is exactly the identity and adds no derivative
# approximation or mutable cache.
#' @keywords internal
.K2_solution_guard_atomic <- local({
  forward <- function(x) {
    ...
    if (is.null(singular_values) || any(!is.finite(singular_values)) ||
        max(singular_values) <= 0) return(bad)
    rho_K <- (min(singular_values) / max(singular_values))^2
    condition_tolerance <- 64 * max(1L, m) * .Machine$double.eps
    if (!is.finite(rho_K) || rho_K <= condition_tolerance) return(bad)
    ...
    for (j in seq_len(nrhs)) {
      ...
      eta <- if (denominator == 0) 0 else max(abs(residual[, j])) / denominator
      if (!is.finite(eta) || eta > backward_tolerance ||
          max(eta, roundoff_floor) / rho_K >= 1) return(bad)
    }
    solution
  }
  ...
  reverse <- function(x, out, out_bar) {
    x <- RTMB::AD(x)
    out_bar <- RTMB::AD(out_bar)
    solution_length <- length(out)
    solution_index <- 2L + seq_len(solution_length)
    c(
      0 * x[1L],
      0 * x[2L],
      out_bar + 0 * x[solution_index],
...
.K2_factor_solve <- function(K2_factor, tvec, parameter_vector, rhs) {
  ...
  R <- .K2_solve_root_guard(factorization$R)
  ...
  solution <- .K2_solution_guard(solution, R, w, z)
  if (!inherits(solution, "advector") && any(!is.finite(solution))) {
    stop("K2 solve failed numerical residual certification.", call. = FALSE)
  }
  solution
}
