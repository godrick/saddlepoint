...
#' If \code{iidReps} is \code{"any"}, the number of blocks is inferred from the input.
#' It requires \eqn{\mathrm{length}(tvec)} to be a multiple of
#' \eqn{d = \mathrm{length}(prob\_vec(\theta))} (the category dimension),
#' and then sets \eqn{m = \mathrm{length}(tvec)/d}.
#'
#'
